...
Sattee.loadVideo = function(path,callback){
    ...
    //attach an event to it for playing and pausing
    newVid.addEventListener("play",function(){
        (function loop() {
          if (!newVid.paused && !newVid.ended) {
            ...
            requestAnimationFrame(loop);
              ...
              console.log(newVid.paused);
              console.log(newVid.ended);
          }
        })();
    });
...
(function () {
    ...
    Sattee.draw = function (layer, func) {
        ...
        Sattee.getCanvas(layer)[0].beginPath();
 ...
 */
    ...
    Sattee.video = function(videoName,x,y, w, h, options){
        ...
        //get the video from the assets
        let vid = Sattee.assets[videoName];
        ...
        _manageOptions("video",options,{
            "ended": ["function",() => {vid.onended = function(){ options.ended() }}],
            "time": ["number",() => {vid.currentTime = options.time}],
            "status":["string",() => {
                if(options.status == "play"){
                    vid.canvasLayer = Sattee.currentLayer;
                    vid.givenX = x;
                    vid.givenY = y;
                    vid.givenW = w;
                    vid.givenH = h;
                    vid.givenCtx = ctx;
                    vid.givenLayer = Sattee.currentLayer;
                    vid.givenOptions = options;
                    if(options.frameRate && typeof options.frameRate == "number")
                    vid.givenFPS = options.frameRate;
                    vid.play();
                }
