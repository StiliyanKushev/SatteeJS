 ...
 */
    ...
    Sattee.image = function (imageName, x, y, w, h, options) {
        ...
        Sattee.validate("image")
            .arguments([imageName, x, y], 3)
            .type([imageName, "string", x, "number", y, "number"]);
        ...
        let img = Sattee.assets[imageName];

        //get the current canvas context
        let [ctx] = Sattee.getCanvas();

        //draw the image
        let hasW = w != undefined && typeof w == "number";
        let hasH = h != undefined && typeof h == "number";
        if(hasW && hasH)
        ctx.drawImage(img, x, y,w,h);
        else if(hasW)
        ctx.drawImage(img, x, y,w);
        else if(hasH)
